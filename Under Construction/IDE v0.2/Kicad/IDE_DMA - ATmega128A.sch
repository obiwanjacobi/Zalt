...
F 1 "74AHC573 (fast)" H 8200 3190 50  0000 C CNN
...
(positive logic)\nOutIntVect = M1 & IOREQ\nRegWr = IOREQ & WR & A[3..7]\nRegRd = IOREQ & RD & A[3..7]\nWait = (RegRd | RegWr) & IOEnable
Wire Wire Line
...
~RESET
Text Label 6540 7290 0    50   ~ 0
Wait
Text Label 6830 3200 0    50   ~ 0
IOEnable
Text Label 5000 8400 0    50   ~ 0
IOEnable
Text Notes 3650 6000 0    50   ~ 0
OutIntVect handling checks if an \ninterrupt was issued then outputs \nthe vector or instruction. \nOtherwise it does nothing.\nOne MCU pin and one register \naddress can be saved by \nputting the vector in a register\nand implementing the logic\nin the GAL.
Text Notes 8900 1500 0    50   ~ 0
Although ATmega128A datasheet\nsays to use a fast AHC latch, I have\nit working with a plain HCT type.
Text Notes 8950 3900 0    50   ~ 0
If we take a bigger MCU that does not require multiplexing pins\nand allows for both DMA/XMEM and IDE to be active at the same \ntime, we do not have to buffer the IDE data inside the MCU's\ninternal RAM, but could write out directly to the target buffer -\nor read directly from the target buffer into the IDE disk.\n- ATmega2560 (used in Arduino Mega)
$EndSCHEMATC
